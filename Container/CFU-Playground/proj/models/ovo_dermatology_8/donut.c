	...
	for (int j = 0; j < combinations; j++) {
		SV_create_env(0,0);
           ...
           for (int k = 0; k < 8; k++) {
                ...
                a |= ((samples[i][k*4] & 0xF));
                a |= ((samples[i][k*4] & 0xF) << 4);
                a |= ((samples[i][k*4 + 1] & 0xF) << 8);
                a |= ((samples[i][k*4 + 1] & 0xF) << 12);
                a |= ((samples[i][k*4 + 2] & 0xF) << 16);
                a |= ((samples[i][k*4 + 2] & 0xF) << 20);
                a |= ((samples[i][k*4 + 3] & 0xF) << 24);
                a |= ((samples[i][k*4 + 3] & 0xF) << 28);

                // Constructing Operand B
                b |= ((weights_q[j][k*4] & 0xFF));
                b |= ((weights_q[j][k*4 + 1] & 0xFF) << 8);
                b |= ((weights_q[j][k*4 + 2] & 0xFF) << 16);
            ...
            // Constructing Operand A
            a |= ((samples[i][32] & 0xF));
            a |= ((samples[i][32] & 0xF) << 4);
            a |= ((15 & 0xF) << 8);
            a |= ((15 & 0xF) << 12);

            // Constructing Operand B
            b |= ((weights_q[j][32] & 0xFF));
            b |= ((biases_q[j] & 0xFF) << 8);
