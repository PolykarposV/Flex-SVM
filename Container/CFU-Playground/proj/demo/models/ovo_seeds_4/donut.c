...
void donut() {
	...
	for (int j = 0; j < combinations; j++) {
            ...
            a |= ((samples[i][1] & 0xF) << 4);
            a |= ((samples[i][2] & 0xF) << 8);
            a |= ((samples[i][3] & 0xF) << 12);
            a |= ((samples[i][4] & 0xF) << 16);
            a |= ((samples[i][5] & 0xF) << 20);
	    a |= ((samples[i][6] & 0xF) << 24);
            a |= ((15 & 0xF) << 28);

            // Constructing Operand B
            b |= ((weights_q[j][0] & 0xF));
            b |= ((weights_q[j][1] & 0xF) << 4);
            b |= ((weights_q[j][2] & 0xF) << 8);
            b |= ((weights_q[j][3] & 0xF) << 12);
            b |= ((weights_q[j][4] & 0xF) << 16);
            b |= ((weights_q[j][5] & 0xF) << 20);
            b |= ((weights_q[j][6] & 0xF) << 24);
	    b |= ((biases_q[j] & 0xF) << 28);

            decision = SV_res4(a,b);
	    print_binary("Decision is", decision);
