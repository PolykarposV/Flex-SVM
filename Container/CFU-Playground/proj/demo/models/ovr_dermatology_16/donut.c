    ...
    for (int i = 0; i < num_samples; i++) {
        ...
        for (int j = 0; j < num_classes; j++) {
            for (int k = 0; k < 16; k++) {
                ...
                a |= ((samples[i][k*2] & 0xF));
                a |= ((samples[i][k*2] & 0xF) << 4);
                a |= ((samples[i][k*2] & 0xF) << 8);
                a |= ((samples[i][k*2] & 0xF) << 12);
                a |= ((samples[i][k*2 + 1] & 0xF) << 16);
                a |= ((samples[i][k*2 + 1] & 0xF) << 20);
                a |= ((samples[i][k*2 + 1] & 0xF) << 24);
                a |= ((samples[i][k*2 + 1] & 0xF) << 28);

                // Constructing Operand B
                b |= ((weights_q[j][k*2] & 0xFFFF));
            ...
            a |= ((15 & 0xF) << 8);
            a |= ((15 & 0xF) << 12);
            a |= ((samples[i][32] & 0xF) << 16);
            a |= ((samples[i][32] & 0xF) << 20);
            a |= ((samples[i][32] & 0xF) << 24);
            a |= ((samples[i][32] & 0xF) << 28);
            // Constructing Operand B
            b |= ((biases_q[j] & 0xFFFF));
            b |= ((weights_q[j][32] & 0xFFFF) <<16);
