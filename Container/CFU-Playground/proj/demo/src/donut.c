   ...
   for (int i = 0; i < num_samples; i++) {
        ...
        for (int j = 0; j < num_classes; j++) {
         for (int k = 0; k < 4; k++) {
                ...
                a |= ((samples[i][k*8] & 0xF));
                a |= ((samples[i][k*8 + 1] & 0xF) << 4);
                a |= ((samples[i][k*8 + 2] & 0xF) << 8);
                a |= ((samples[i][k*8 + 3] & 0xF) << 12);
                a |= ((samples[i][k*8 + 4] & 0xF) << 16);
                a |= ((samples[i][k*8 + 5] & 0xF) << 20);
                a |= ((samples[i][k*8 + 6] & 0xF) << 24);
                a |= ((samples[i][k*8 + 7] & 0xF) << 28);

                // Constructing Operand B
                b |= ((weights_q[j][k*8] & 0xF));
                b |= ((weights_q[j][k*8 + 1] & 0xF) << 4);
                b |= ((weights_q[j][k*8 + 2] & 0xF) << 8);
                b |= ((weights_q[j][k*8 + 3] & 0xF) << 12);
                b |= ((weights_q[j][k*8 + 4] & 0xF) << 16);
                b |= ((weights_q[j][k*8 + 5] & 0xF) << 20);
            ...
            //print_binary("Current max index is ", max_idx);
        }
